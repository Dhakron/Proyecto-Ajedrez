...
public class Piece {
	...
	public ArrayList<Action> getDiagonalUpLeftMoves(State state) {
		...
		Boolean busyCell = false;
		for (int r = r0 - 1, c = c0 - 1; (r >= 0) && (c >= 0) && !busyCell; r--, c--) {
			if (state.m_board[r][c] == Utils.empty) {// add action
				action = new Action(state.m_agentPos, new Position(r, c));
				list.add(action);
			} else {
	...
	public ArrayList<Action> getDiagonalDownRightMoves(State state) {
		...
		Boolean busyCell = false;
		for (int r = r0 + 1, c = c0 + 1; (r < state.m_boardSize) && (c < state.m_boardSize) && !busyCell; r++, c++) {
			if (state.m_board[r][c] == Utils.empty) {// add action
				action = new Action(state.m_agentPos, new Position(r, c));
				list.add(action);
			} else {
	...
	public ArrayList<Action> getDiagonalDownLefttMoves(State state) {
		...
		Boolean busyCell = false;
		for (int r = r0 + 1, c = c0 - 1; (r < state.m_boardSize) && (c >= 0) && !busyCell; r++, c--) {
			if (state.m_board[r][c] == Utils.empty) {// add action
				action = new Action(state.m_agentPos, new Position(r, c));
				list.add(action);
			} else {
